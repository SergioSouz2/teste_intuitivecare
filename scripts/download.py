...
def get_last_trimesters(url: str, limit=3) -> List[Tuple[str, str]]:
    ...
    years = sorted(
        [int(a["href"].strip("/")) for a in soup.find_all("a", href=True)
    ...
    for year in years:
        ...
        for a in s.find_all("a", href=True):
            if a["href"].endswith(".zip") and a["href"][0].isdigit():
                m = re.search(r'([1-4])T', a["href"])
                if m:
                    q = int(m.group(1))
                all_files.append((a["href"], f"{y_url}{a['href']}", year, q))

    all_files.sort(key=lambda x: (x[2], x[3]), reverse=True)
    return [(f[0], f[1]) for f in all_files[:limit]]
